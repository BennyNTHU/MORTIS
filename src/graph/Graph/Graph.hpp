...
class Graph 
{
    private:
        ...
        std::vector<std::vector<int>*> edgeVector; // Each edge is represented as a vector of three ints: {u, v, w}.
    ...
    public:
         ...
         */
        template <typename U>
        friend std::ostream& operator<<(std::ostream& out, const Graph<U>& graph);
         ...
         */
        template <typename U>
        friend std::istream& operator>>(std::istream& in, Graph<U>& graph);
};
