    ...
    for (auto nbrIt = it->begin(); nbrIt != it->end(); ++nbrIt) {
        ...
        if (disc[v] == -1) {  // v not visited.
            ...
            if ((parent[u] == -1 && children > 1) || (parent[u] != -1 && low[v] >= disc[u])) {
                ...
                if (!st.empty()) { // pop the (u,v) edge too.
                    auto edge = st.top();
                    st.pop();
                    compSet.insert(edge.first);
                    compSet.insert(edge.second);
...
Graph Graph::Prim() {
    ...
    for (int i = 0; i < 0; i++) ++it0; // already at node 0.
...
SparseMatrix Graph::AdjMatSparse() const {
    ...
    for (int i = 0; i < edgelist.length(); i++) {
        ...
        // Use StoreNum to store the weight (converted to float) at (u, v).
        mat.StoreNum(std::variant<int, float>(static_cast<float>(weight)), u, v);
        // For undirected graphs, also store the symmetric term.
        if (!isDirected)
            mat.StoreNum(std::variant<int, float>(static_cast<float>(weight)), v, u);
    }
    return mat;
}
